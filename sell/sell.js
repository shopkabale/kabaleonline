...
// All element selections from your original file
...
function updateCategoryOptions() {
    if (!categorySelect) return; // Safety check for hostel page
...
onAuthStateChanged(auth, async (user) => {
    const isOnSellPage = !!document.getElementById('product-form-container');
        ...
        if (user.emailVerified) {
            ...
            if (isOnSellPage) {
                ...
                if (!userDoc.exists()) {
                    ...
                    if (completeProfileSection) completeProfileSection.style.display = 'block';
                } else {
                    ...
                    } else if (completeProfileSection) { completeProfileSection.style.display = 'none'; }
...
if (signupForm) {
    signupForm.addEventListener('submit', async (e) => {
        ...
        const whatsappInput = document.getElementById('signup-whatsapp'); // This is the key change
        const whatsapp = whatsappInput ? whatsappInput.value : ''; // Safely get value
        ...
        if (!name || !email || !password) { return showMessage(signupErrorElement, "Please fill out name, email, and password."); }
        ...
        try {
            ...
            // This object now safely includes all possible fields from both sell and hostel pages
            const userData = { 
                name, email, role: 'seller', isVerified: false, 
                createdAt: serverTimestamp(), referralCount: 0, referrerId: referrerId,
