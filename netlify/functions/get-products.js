...
exports.handler = async (event) => {
  ...
  const { 
    pageSize, page, searchTerm, category, sale,
  ...
  const fieldIds = {
    status: 'field_5235549', name: 'field_5235543', category: 'field_5235550',
    isOnSale: 'field_5235557', isFeatured: 'field_5235554', isSponsored: 'field_5235555',
    isVerified: 'field_5235556'
  };

  const conditions = [];
  
  // --- THE ONLY CHANGE IS HERE: The problematic Status filter is now disabled ---
  // conditions.push({ type: 'equal', field: fieldIds.status, value: 'Approved' });

  // All other filters will still work
  if (searchTerm) {
  ...
  const queryParams = new URLSearchParams({
    user_field_names: true, size: size, page: pageNumber, order_by: '-id'
  });

  if (conditions.length > 0) {
    const filtersObject = { filter_type: 'AND', filters: conditions };
